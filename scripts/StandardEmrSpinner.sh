...
clusterName="$(aws emr create-cluster --name "Standard Pipeline EMR cluster"  --release-label emr-4.0.0 --applications Name=Spark --ec2-attributes KeyName=DatomataClusterKeyPair --instance-type m3.xlarge --instance-count 1 --use-default-roles --query 'ClusterId' --output text)"
