...
clusterName="$(aws emr create-cluster --name "Count Pipeline EMR cluster"  --release-label emr-4.2.0 --applications Name=Spark --ec2-attributes KeyName=DatomataClusterKeyPair --instance-type m3.xlarge --instance-count 5 --use-default-roles --query 'ClusterId' --output text)"
...
spark-submit --class kiwi.Knitter ./Kiwi-1.0.0.jar _ PunjabKesari count
ENDSSH
chmod 777 *
